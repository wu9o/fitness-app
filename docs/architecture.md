...
保持为空，不从路线或速度推测。与运动关联的心率样本通过 `predicateForObjects(from:)` 查询，最多下采样
为 360 个展示点并持久化；重复同步同一 UUID 时更新设备指标和采样，但保留用户填写的主观体感。
详情页展示真实采样曲线；`TrainingProfileStore` 在本地保存用户确认的最大心率，设置后按最大心率
的 60%、70%、80% 和 90% 边界展示个性化 5 区，未设置时明确降级为固定 bpm 分段。超过 30 秒
的采样空档不补算。HealthKit 不可用、
...
只在状态变化时提醒，到达终点只提醒一次。Flutter 层根据本地设置分别调用系统触觉和中文 TTS；
两个通道可独立关闭，TTS 在播报新提示前停止上一条，避免 GPS 连续采样造成语音堆积；
语音默认关闭，需由用户在设置中主动开启。
